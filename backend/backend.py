    ...
    try:
        ...
        with open(output_file_path, 'w', encoding='utf-8') as output_file:
            for cluster in clusters:
                command = f"trivy k8s --report summary cluster --kubeconfig {kubeconfig_path} --context {cluster}"
...
def start_scan():
    ...
    output_file_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), "scan_results.txt")
    ...
    response = jsonify({'scan_result': scan_result})
